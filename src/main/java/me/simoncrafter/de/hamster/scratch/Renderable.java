...
/**
 * Die Klasse Renderable ist das Herzst�ck des Programms, denn diese beschreibt
 * die einzelnen Elemente, die verschoben und zusammengesteckt werden k�nnen.
 * Die Klasse bietet hilfreiche Methoden, um erfolgreich zu entscheiden, ob bei
 * einem Mausklick dieses Objekt getroffen wurde, oder beim verschieben dieses
 * Objekt an ein anderes angedockt werden kann. Desweiteren bietet es
 * verschiedene Andockmethoden, die sofort veranlasst die Vaterelemente und
 * Kinderelemente neu zu berechnen und neu zu rendern.
 * 
 * @author HackZ
 * 
 */
public abstract class Renderable {
	...
	protected ScratchHamster hamster = ScratchHamster.hamster;
	 ...
	 * Attribut name zur�ckgeliefert. Dieser wird auch verwendet, um das Objekt
	 * zu speichern. Sollte man etwas anderes anzeigen wollen, als gespeichert
	 * werden soll, so muss diese Methode �berschrieben werden und einen festen
	 * Wert zur�ckliefern.
	 * 
	 * @return
	 */
	 ...
	 * dieses Renderable gesetzt (dies muss vom Renderer als Referenz �bergeben
	 * werden)
	 * 
	 * @param r
	 ...
	 *         <li>-2: Das �bergebene Renderable kann als Parent an das aktuelle
	 *         angebunden werden</li>
	...
	public int dockingTest(Renderable r, ScratchPanel renderer) {
		...
		if (r.getType() != RType.BOOLEAN && parent == null && parentDock != null
				&& r.isParentable()) {
	 ...
	 * Picking). Es werden auch alle childs �berpr�ft und das selectierte object
	 * returnt
	 * 
	 * @param x
	 *            x Koordinate
	...
	/**
	 * Das �bergebene child wird in dieses Renderable am Andockpunkt hinzugef�gt
	 * 
	 * @param child
	...
	/**
	 * F�gt das �bergebene Renderable-Objekt als next-Element an dieses Objekt
	 * an. Sollte dieses Element einen Nachfolger haben, so wird das �bergebene
	 * Renderable dazwischen geschaltet.
	 * 
	 * @param child
	 */
	...
	/**
	 * Liefert f�r dieses Renderable das Wurzelelement, an dem alles h�ngt, oder
	 * dieses Element, wenn es kein Vaterelement besitzt (und somit das
	 * Wurzelelement ist).
	 * 
	 * @return
	 */
	...
	/**
	 * Sucht in diesem Renderable den �bergebenen child und entfernt diesen als
	 * Referenz
	 * 
	 * @param child
	...
	/**
	 * Updatet die BoundingBoxen f�r dieses Element, abh�ngig von den Parametern
	 * und den Kinderelementen. Da dies f�r jedes Element unterschiedlich ist,
	 * muss jedes abgeleitete Element selbst daf�r sorgen, dass die
	 * gr��enverh�ltnisse richtig angepasst werden.
	 */
	...
	/**
	 * Liefert true zur�ck, wenn das aktuelle Renderable samt Kindern als Parent
	...
	/**
	 * F�hrt die interne Coderepr�sentation dieses Renderables aus. Dabei wird
	 * darauf geachtet, ob das Programm gestoppt oder pausiert wurde.
	 * 
	 * @param program
	 *            Das ScratchProgramm, in derm auch der StorageController samt
	 *            Methoden enthalten ist. Dieser ist zust�ndig f�r Stopp und
	 *            Pause.
	 * @return Liefert null bei void Methoden oder den entsprechenden boolschen
	 *         Wert.
	 * @throws FunctionResultException
	 *             Damit "return" in einer Methode auch sofort die Methode
	 *             verl�sst, wird das Ergebnis in einer Exception �bergeben, die
	 *             beim Aufruf jeder Methode aufgefangen wird.
	 */
	public Object perform(ScratchProgram program)
			throws FunctionResultException {
		// Setze im Programm, dass dieses Renderable aktiv ist
		ScratchProgram.setActive(this);
		...
		// w�hrend einer Pause wird stop gedr�ckt (dibo)
		if (program.checkStop()) {
			return false;
		}
		...
		// Pause und Stopp �berpr�fen
		if (program.checkStop()) {
			return false;
		}

		// N�chstes Element ausf�hren
		if (next != null)
	...
	/**
	 * F�hrt den Befehl von diesem Renderable aus. Jedes Objekt ist selbst
	 * zust�ndig f�r die richtige Ausf�hrung der Funktion.
	 * 
	 * @return
	 */
	public abstract Object performImplementation(ScratchProgram program)
			throws FunctionResultException;
	 ...
	 * @param needsReturn
	 *            Es ist eine Boolean Methode, die am Ende ein return ben�tigt.
	 */
	...
	/**
	 * Schreibt in dem �bergebenen buffer abh�ngig von den Parametern den Anfang
	 * einer Zeile. Schreibt den Kommentarstring und die Anzahl an Tabs (KEINE
	 * Zeilen�berg�nge!!!).
	 * 
	 * @param buffer
	 * @param layer
