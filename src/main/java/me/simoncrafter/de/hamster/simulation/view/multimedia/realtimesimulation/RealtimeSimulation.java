package me.simoncrafter.de.hamster.simulation.view.multimedia.realtimesimulation;
...
import me.simoncrafter.de.hamster.simulation.model.Terrain;
import me.simoncrafter.de.hamster.simulation.view.multimedia.opengl.OpenGLController;
import me.simoncrafter.de.hamster.simulation.view.multimedia.opengl.Scene;
import me.simoncrafter.de.hamster.simulation.view.multimedia.sound.PlayWave2;
import me.simoncrafter.de.hamster.simulation.view.multimedia.sound.SoundClip;
import me.simoncrafter.de.hamster.simulation.view.multimedia.sound.SoundManager;

/**
 * @author chris
 * 
 * Die Echtzeitsimulationskomponente, die aus den diskreten Hamsterpositionen
 * einen Satz an Pixelgenauen Positionen f�r die Darstellung in 3D generiert.
 * 
 */
public class RealtimeSimulation {
	...
	public void interpolatePositions(long time) {
		...
		// weil die OO-Hamster zur laufzeit erstellt und gel�scht werden,
		// m�ssen wir zwangsl�ufig mit �nderungen in der anzahl rehcnen.
		...
		if (this.entries.size() == 0) {
			...
			// in diesem status l�uft keine Simulation, aber die Positionen des
			// Hamsters k�nnen sich durch Usereingaben ver�ndern. also setzen
			...
			while (this.entries.size() != 0) {
				...
				if (moving == false) { // && keine �nderung an den k�rnern!

					// es wurde kein Hamster bewegt oder gedreht. also ist dies
					// ein leer-event oder zeigt, da� ein neuer OO-Hamster
					// erstellt wurde
					// in jedem Fall m�ssen wir hier nichts extra tun, sondern
					// l�schen
					// das event und nehmen uns einfacc mal das n�chste vor,
					// sofern vorhanden.
					...
					// bewegt,
					// und wir m�ssen dieses event abarbeiten. danach loopen wir
					// auch nicht
					// weiter zum n�chsten event.
					...
					// hamster
					// bewegt werden m�ssen:
					float dist = this.speed * (float) delta / 1000f;

					// pr�fen, ob so viel zeit vergangen ist, da� wir das event
					// ganz abhaken k�nnen:
					if (dist + this.movedSinceLastStage >= 1f) {
						...
						// die stage aus der liste l�schen. falls bereits eine
						// weitere
						...
						// erreichen
						// also �berspringen wir sie nicht, sondern latschen
						// mutig los.

						// alle hamster durchlaufen und ihre positionen anhand
						// der
						// richtung aktualisieren:
						// (wir wissen ja genau, da� mindests (und vermutlich
						// genau) einer l�uft)
						for (int i = 0; i < this.entries.get(0)
								.numberOfHamsters(); i++) {
	...
	// einschlie�lich des -1er default hamsters:
