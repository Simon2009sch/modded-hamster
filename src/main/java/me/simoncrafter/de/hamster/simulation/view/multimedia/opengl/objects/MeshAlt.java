...
import me.simoncrafter.de.hamster.simulation.view.multimedia.opengl.math.Vector2f;
import me.simoncrafter.de.hamster.simulation.view.multimedia.opengl.math.Vector3f;
...
public class MeshAlt {
	...
	// die vertexbuffer f�r opengl, diese enthalten jeweils den aktuell interpolierten keyframe: 
	...
	public void draw(GLAutoDrawable gld, long time) {
		...
		// interpolieren, oder sichergehen, da� die buffers mit statischen daten gef�llt sind,
		// je nachdem, ob wir animiert sind, oder nicht:
		if (this.numberOfKeyframes() > 1 ) {			
			// falls die animation l�uft oder noch nix interpoliert wurde:
			if (this.animating) this.interpolate(gl2, time);
			this.buffersBound = false;			
		}
		
		// prima, nun sind die buffers also mit daten zum darstellen gef�llt,
		// sichergehen, da� sie auch gebunden sind:
		if (!this.buffersBound) this.bindBuffers(gl2);
		
		// und nun k�nnen wir die buffer zeichnen lassen:
		
		// material, farben etc setzen:
		// die texturkoordinaten werden ja woanders, n�mlich in den buffern gesetzt.
...
*/
	...
	private void createBuffers() {
		...
		// auf jeden fall die buffer initial mit dem ersten keyframe f�llen
		// bei sgtatischen objekten ist dies das einzige mal, das es geschieht,
		// bei animierten dagegen wird in jedem frame nach dem interpolieren neu gef�llt.
		fillBuffers();
	...
	private void fillBuffers() {
		...
		// �bertrage die vertices, die normals und die texturkoords in die buffer:
		Vector3f v; 
		Vector2f v2; 
...
//		System.out.println("Interpolate");
		...
		// wenn wir mehr zeit darstellen m�ssen, als der gerade offenen keyframe
		// dauert, dann m�ssen wir folglich mindestens einen �berspringen:
		while (timeLeftFromLastKeyframe < timeToInterpolate) {
			
//			System.out.println(" Skipping...");
			
			// die animation anweisen, zum n�chsten Keyframe zu springen:
			// die gibt uns eine aktualisierte Angabe �ber die aktuelle Animation.
			...
			// wir haben durch das springen zum n�chsten frame gewisserma�en die restzeit des alten
			...
			// wenn die schleife nun von neuem beginnt, wird sie pr�fen, ob wir immer 
			// noch mehr zeit zu �berbr�cken haben, als der nun aktuelle keyframe dauert.
			// wenn nicht, dann ist die schleife zuende und wir haben sowohl den aktuellen
			// last-keyframe, als auch eine aktuelle zeit, die in diesem keyframe zu �berbr�cken ist.
			
		}
		
//		System.out.println(" getting last and next");
		
		// so, wenn wir hier sind, ist der teil oder genau der rest eines keyframes zu �berbr�cken:
