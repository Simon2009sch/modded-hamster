 ...
 * @author chris
 * 
 * Enth�lt die Definition der Hamsterwelt und die Methoden, sie zu zeichnen.
...
import me.simoncrafter.de.hamster.simulation.view.multimedia.opengl.objects.Heightmap;
import me.simoncrafter.de.hamster.simulation.view.multimedia.opengl.objects.Mesh;
...
public class HamsterWorld {
	...
	// die objekte f�r den Default- und die Extra-Hamster:
	...
	public void draw(GLAutoDrawable gld, RealtimeSimulation positions, long time) {
		...
		if (model.getTerrain().getWidth() != this.width
			...
			// die karte hat sich ge�ndert, es muss gleich ne neue her:
		...
		// alle hamster im entry durchlaufen. mit -1 f�r den default hamster
		// anfagen:
		Obj h = null;
		for (int i = -1; i < positions.getNumberOfHamsters() - 1; i++) {

			// den jeweiligen Hamster w�hlen. Achtung: Im Multi-Hamster-Modus
			// k�nnen zur Laufzeit neue Hamster entstehen. Wenn wir also noch
			// nicht genug
			// Hamsterobjekte erzeugt haben, m�ssen wir das jetzt tun.
			h = this.hamster;
			if (i >= 0) {
			...
			me.simoncrafter.de.hamster.simulation.model.Hamster ham = model.getHamster(i);
