...
import me.simoncrafter.de.hamster.simulation.model.Terminal;
...
public class DialogTerminal extends JDialog implements Terminal {
	...
	JLabel getMessage(int hamsterid, String string) {
		...
		if (UIManager.getLookAndFeel() == Workbench.startLAF) {
			int color = -2;
			SimulationModel model = me.simoncrafter.de.hamster.workbench.Workbench
	...
	public void write(Throwable e) {
		...
		for (int i = 0; i < ste.length - 1; i++) {
			if (ste[i].getClassName().equals(
					"me.simoncrafter.de.hamster.debugger.model.Hamster"))
				continue;
			if (ste[i].getClassName().equals(
					"me.simoncrafter.de.hamster.debugger.model.IHamster"))
				continue;
