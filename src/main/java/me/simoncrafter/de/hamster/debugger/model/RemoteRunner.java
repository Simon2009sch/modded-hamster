...
import me.simoncrafter.de.hamster.lego.model.LHamster;
import me.simoncrafter.de.hamster.model.HamsterProgram;
...
public class RemoteRunner {
	...
	/**
	 * Mit diesem Konstruktor wird der RemoteRunner auf Server-Seite erzeugt. Dieser
	 * erzeugt direkt den serverseitigen RemoteProcessor.
	 * 
	 * @param processor
	 *            An diesen Processor muessen die vom Client kommenden Befehle
	 *            weitergeleitet werden
	 * @param p
	 *            Dies ist der Prozess, in dem die Client-VM lauuft. Ueber dieses
	 *            Objekt koennen die Ein- und Ausgabestroeme verbunden werden.
	 */
	public RemoteRunner(InstructionProcessor processor, Process p) {
		remoteProcessor = new RemoteProcessor(p.getInputStream(), p.getOutputStream(), processor);
	}
	...
	/**
	 * Dies ist die main-Methode, die aufgerufen wird, wenn ein Hamsterprogramm in
	 * einer Client-VM ausgefuehrt wird.
	 * 
	 * @param args
	...
	public static void main(String[] args) {
		...
		try {
			...
			try {
				...
				System.setErr(new PrintStream(new FileOutputStream("syserr.txt")));
			...
			try {
				...
				if (Utils.security) { // dibo
					...
					//System.setSecurityManager(new SecurityManager());
					// dibo 21.12.2017 funktioniert nicht in Java SE 9
				}
				hamster.main();
			} catch (Exception e) {
