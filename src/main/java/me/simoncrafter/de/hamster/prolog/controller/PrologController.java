package me.simoncrafter.de.hamster.prolog.controller;
...
import me.simoncrafter.de.hamster.prolog.model.PrologHamster;
import me.simoncrafter.de.hamster.prolog.model.PrologQuery;
 ...
 * repr�sentativ f�r den Standard-Hamster des Hamstersimulators, und die
 * {@link PrologKonsole}. Die {@link PrologKonsole} bietet eine alternative
 * M�glichkeit zur Kommunikation mit dem Prologinterpreter sowie weitere, f�r
 * das Debugging(Tracing) hilfreichen, unterst�tzenden Werkzeuge.
 * 
...
public class PrologController {
	 ...
	 * Prolog-Sitzung. Die Kommunikation mit dem Prolog-Interpreter erfolgt �ber
	 ...
	 * Unwichtige/redundante Ausgaben des Interpreters k�nnen dadurch
	 * ausgeblendet werden.
	 */
	...
	/**
	 * Nach dem Ausf�hren (Abschicken) einer einzelnen Anweisung (im trace
	 * Modus) sollte man Richtigerweise erst auf die R�ckmeldung von Prolog
	 * warten, bevor man die n�chste Anweisung annimt und abschickt.
	 */
	 ...
	 * gestartet, mit zus�tzlichen Initialisierungsparametern.
	 */
	public synchronized boolean startPrologEngine() {
		...
		try {
			...
			if (s != null) {
				 ...
				 * zus�tzlicher Informationen -f none -> verwende keine
	...
	public boolean ensurePrologEngineIsRunning() {
		...
		/*
		 * Falls die Prolog-Engine nicht mehr l�uft, starte diese neu und
		 * initialisiere auch die �brigen Komponenten..
		 */
		if (!isRunning) {
			prologEngineStarted = false;
			boolean engineStarted = startPrologEngine();
			if (engineStarted) {
				// Initialisiere bzw. setze zur�ck die PrologKonsole.
				PrologKonsole.get().reset();
				// Warte bis der SWIProlog-Banner vollst�ndig eingeblendet ist.
	...
	/**
	 * Vor der Auf�hrung einer jeden neuen Query, sollte gewartet werden bis die
	...
	public HashMap<String, String> execute(String command) {
		...
		// Warte bis diese Query ausgef�hrt ist.
		try {
	 ...
	 * unterst�tzt dass die Ausgabezeilen von Prolog je nach aktuellem Kontext
	 * mit entsprechenden Zeilenpr�fixen versehen sind. Danach kann der
	...
	public void handlePrologOutput(String line,
		 ...
		 * 'message_hook.pl' definierten Schl�sselw�rtern. Reagiere aber nicht
		 ...
		 * wurde. In der 'message_hook.pl' ist vor jedem Schl�sselwort der
		...
		// Verarbeite aktuelle Zeile anhand der vordefinierten Pr�fixe.
		else {
			if (line.startsWith("SILENT:")) {
				...
				/*
				 * Pr�fe nach, ob der'trace'- oder 'debug'-Modus sich ge�ndert
				 * hat.
				 */
			 ...
			 */
				...
				/*
				 * Solle die Ausf�hrung der Query abgebrochen sein, muss dies an
				 ...
				 * PrologEngine erh�ht werden. F�r die entsprechend korrekte
			 ...
			 */
				 ...
				 * entsprechende Fehlermeldung f�r den Benutzer einzublenden.
				 */
				if (prologQuery != null) {
				 ...
				 * werden, unabh�nging davon, welchen Wert die
			...
			/*
			 * Sonstige Ausgaben: Dazu geh�ren auch die Statusmeldungen zur
			 ...
			 */
				...
				/*
				 * Im Trace-Mode macht plcon merkw�rdigerweise hin und wieder
				...
				if (showOutput) {
					/*
					 * F�hre nachfolgend einige Umformatierungen der
					 * Prolog-Ausgaben durch und leite diese anschlie�end an die
					 ...
					 * einen abschlie�enden Punkt) oder ein '?' aufweisen, so
					 * handelt es sich hierbei um eine Frage seitens der
					 * Prolog-Engine. F�ge nachfolgend, zur Verdeutlichung
			 ...
			 * vorangestellten Schl�sselwort.
			 */
			else {
				// System.out.println("betrete block OHNE PR�FIX..");
				...
				if (line.startsWith("prologhamster:")
						&& !line.contains("write")) {
					/*
					 * Nach der Ausf�hrung eines jeden Hamsterbefehls erwartet
					 * der Prolog- Interpreter ein R�ckgabewert (bzw. ein Term).
					 * In unseren Fall wird dies entweder ein 'true.' oder ein
					 * 'false.' sein. Ein 'true.' sollte signalisieren, dass die
					 * Ausf�hrung des Hamsterbefehls ohne eine Exception
				...
				/*
				 * Zum Warten auf die Ausf�hrung eines Hamsterbefehls wird auf
				 * Prolog-Seite das 'read(Term)'-Kommando verwendet. Dieses
				 * macht aber automatisch eine "|:"-Ausgabe auf die Konsole.
				 * Damit der Benutzer zuvor erw�hnte Ausgabe an dieser Stelle
				 ...
				 */
					 ...
					 * in diesem Fall bei der Aktionsausf�hrung nicht auf der
					 ...
					 * eingeleitet, worauf die PrologKonsole f�r
				...
				/*
				 * Alle �brigen Ausgaben sollten einfach direkt an die
				 ...
				 */
					...
					if (showOutput) {
						if (line.length() > 0) {
							...
							// Nach einem Fehler lasse am Ende den Query-Pr�fix
							// einblenden..
	...
	private void updateSettings() {
		/*
		 * Nach einer abgeschlossenen Querry soll der Error-Flag zur�ckgesetzt
		 * werden.
		 */
	...
	/**
	 * Erstellt eine tempor�re Datei zur initialisierung des
	 * Prolog-Interpreters.
	 * 
	...
	/**
	 * Liefert die die Referenz auf den Prolog-Prozess zur�ck.
	 */
