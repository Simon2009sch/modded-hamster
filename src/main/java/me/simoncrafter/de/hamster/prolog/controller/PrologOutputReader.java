 ...
 * diese an den {@link PrologController} weiter. Dabei wird f�r jede
			...
			try
			{
				if(inputStream.available() > 0)
				{
					...
					for (int i = 0; i < lines.length; i++)
					{
						...
						{
							/*
							 * Pr�fe ob ein HamsterBefehl mit dabei ist. Falls ja, behandle 
							 * ihn gesondert..
							 */
							String line = lines[i];
							if(line.contains("prologhamster:"))
							{
								...
								// Pr�fe nach, dass dies keine 'listing'-Ausgabe ist.
							...
							// Eine Zeile die defenitiv kein Hamsterbefehl enth�lt..
							else
							{
								String nextLine = ((i + 1) < lines.length) ? lines[i + 1] : null;
								PrologController.get().handlePrologOutput(lines[i], true, nextLine);
